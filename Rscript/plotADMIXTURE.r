...
tiff(file=paste0(opt$outPrefix,".tiff"),width = 4000, height = 5800,res=400)
...
df_file=paste0(opt$outPrefix,".cv.error")
df <- read.table(df_file, sep = " ", head=T)

df_plot <- ggplot(df, aes(x=K,y=CV_Error))+geom_point()+geom_line()
ggsave(paste0(opt$outPrefix,'.jpeg'), df_plot, dpi=300)
