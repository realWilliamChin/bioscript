...
} else if (!dir.exists(opt$datadir)) {
  print_help(opt_parser)
  stop("datadir 目录不存在，请输入正确的 datadir 目录")
}
...
if (substr(opt$datadir, nchar(opt$datadir), nchar(opt$datadir)) != "/") {
  opt$datadir <- paste0(opt$datadir, "/")
}
...
go_enrich <- function(deg_id_file, output_dir) {
  ...
  # go_bp_bar_filename = sub('.txt', '_EnrichmentGO_BP_Barplot.png')
  # go_cc_bar_filename = sub('.txt', '_EnrichmentGO_CC_Barplot.png')
  # go_mf_bar_filename = sub('.txt', '_EnrichmentGO_MF_Barplot.png')

  # png(go_bp_bar_filename, width = 800, height=600)
  # barplot(go_rich_bp, showCategory=15, title="EnrichmentGO_BP")
  # dev.off()

  # png(go_cc_bar_filename, width = 800, height = 600)
  # barplot(go_rich_cc, showCategory=15, title="EnrichmentGO_CC")
  # dev.off()

  # png(go_mf_bar_filename, widht = 800, height = 600)
  # barplot(go_rich_mf, showCategory=15, title="EnrichmentGO_MF")
  # dev.off()
}
