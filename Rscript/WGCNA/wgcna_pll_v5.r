setwd("L:/work/01_Refenrence/2023_10_16_xiaomai/2023_11_16_WGCNA/WGCNA_1")
library(DESeq2)
library(WGCNA)
library(genefilter)
library(tidyr)
...
ggsave("Analysis_color_module_barplot/normalized_exp_violin.pdf",exp_violin.plot,dpi=320)
...
###################################################
pdf(file = "Raw_data/wgcna-01hclust.pdf", width = 12, height = 9);
...
#结果作图
pdf(file="Raw_data/wgcna-02-1.pdf",width=9,height=5) 
par(mfrow = c(1,2));
cex1 = 0.8;  
...
pdf(file="Raw_data/wgcna-02-2.pdf",width=12,height=9) 
...
#windows()
#sizeGrWindow(12,9)
pdf(file="Analysis_color_module_barplot/2ndwgcna-02-2.pdf",width=12,height=9)
...
plotTOM = dissTOM^7;
diag(plotTOM) = NA;
tiff(file="Analysis_color_module_barplot/fpkm-heatmap.tif")
...
png("Analysis_color_module_barplot/dynamic_tree_cut.png",width = 800,height = 600)
...
png("Analysis_results/Clustering_of_module_eigengenes1_0.21.png",width = 800,height = 600)
...
dev.off()
#绘制两两模块间的邻接矩阵
png("Analysis_color_module_barplot/wgcna.adjacency.heatmap.png",height = 1000,width = 900)
...
for(module in substring(colnames(MEs),3)){
  ...
  cyt = exportNetworkToCytoscape(modTOM,edgeFile = paste("Cytoscape_all/","CytoscapeInput-edges-", module, ".txt", sep=""),
                                 nodeFile = paste("Cytoscape_all/","CytoscapeInput-nodes-", module, ".txt", sep=""),
                                 weighted = TRUE,threshold = 0.2,nodeNames = modProbes,nodeAttr = moduleColors[inModule])
}
##############################################################################################################
dir()
edge_files<-grep("CytoscapeInput-edges-",dir("Cytoscape_all"),value = T)
class(edge_files)
weight_value<-NULL
weight_value_df<-as.data.frame(matrix(1:8,nrow=1))
i<-NULL
for (i in 1:length(edge_files)) {
  temp_df <-read.table(paste0("Cytoscape_all/", edge_files[i]),header=T,sep="\t",check.names = F,quote='',stringsAsFactors = F)
...
write.table(weight_value_df,"Analysis_results/edge_weighted_value.txt",sep="\t",quote=F,row.names=F)
weight_value_df
#WeightValue_60<-quantile(weight_value,0.6)
par(mar=c(5.1,4.1,4.1,2.1))
pdf("Analysis_results/histogram_edge_weight_value.pdf",width=9,height=5)
hist(weight_value,breaks=100,col = "light blue")
dev.off()
png("Analysis_results/histogram_edge_weight_value.png",height = 1000,width = 900)
hist(weight_value,breaks=100,col = "light blue")
dev.off()
for (sub_edge_file in edge_files) {
  edge_tmp_df<-read.table(paste0("Cytoscape_all/", sub_edge_file),header=T,sep="\t",check.names = F,quote='',stringsAsFactors = F)
  ...
  sub_edge_file_out<-paste0("Analysis_results/",sub_edge_file,"_Top100.txt",sep="")
  ...
  node_tmp_df<-read.table(paste0("Cytoscape_all/",sub_node_file),header=T,sep="\t",check.names = F,quote='',stringsAsFactors = F)
  ...
  sub_node_file_out<-paste0("Analysis_results/",sub_node_file,"_Top100.txt",sep="")
