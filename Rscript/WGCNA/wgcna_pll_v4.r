setwd("d:/pll/R_work/WGCNA/five_group/WGCNA_FPKM/Arctium_lappa/")
library(DESeq2)
library(WGCNA)
library(genefilter)
library(tidyverse)
...
#########################################################################################################
#head(node_freq)
AllNodes<-read.table("All_Gene_KME.txt",sep="\t",header=F,stringsAsFactors = F,check.names = F,quote='')
AllNodes[3]
NodeStat.df<-as.data.frame(table(AllNodes[3]))
colnames(NodeStat.df)<-c("Module","Counts")
write.table(NodeStat.df,"Nodes_summary.txt",sep="\t",row.names=F,quote=F)