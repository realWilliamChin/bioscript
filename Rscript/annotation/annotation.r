...
Swiss<-read.table("Swisss_ID.list",sep="\t",header=F,stringsAsFactors = F)
...
data.kegg<-read.table("KEGG_clean.txt",sep="\t",stringsAsFactors = F,header=F)
...
BP<-read.table("GO_BP.txt",sep="\t",header=F,stringsAsFactors = F)
#head(BP)
#table(head(BP[,2]))
BP.count<-data.frame(table(BP[,2]))
BP.20 <- head(BP.count[order(-BP.count[,2]),],n=20)

CC<-read.table("GO_CC.txt",sep="\t",header=F,stringsAsFactors = F)
CC.count<-data.frame(table(CC[,2]))
CC.20 <- head(CC.count[order(-CC.count[,2]),],n=20)
#CC.20
#?read.table
MF<-read.table("GO_MF.txt",sep="\t",header=F,stringsAsFactors = F,quote ="")
