setwd("C:/Users/Analysis/OneDrive/Work/zhiwusuo/01_Work/2023_05_09_跑R")
library(ggthemes)
library(ggplot2)
library(pheatmap)
library(reshape2)
...
bs_pos<-1
bs_neg <- -bs_pos
read.table("Genus_proportion_summary.txt",sep="\t",header=T,row.names=1,check.names=F)->fpkm

all_fpkm<-fpkm[rowSums(fpkm)>0,]
all.heatmap<-pheatmap(all_fpkm,scale="row",cluster_cols=F,show_rownames=F)
ggsave("all_heatmap.jpeg",all.heatmap,dpi=300,width=10,height=10)

sample_info<-read.table("GZ_sample_des.txt",sep="\t",header=T,check.names=F,stringsAsFactors = F)
#sample_info[sample_info$group=="CK",]
reads_data<-read.table("Genus_summary.txt",sep="\t",row.names=1,header=T,check.names=F,stringsAsFactors = F)
...
for(i in seq_along(1:nrow(comp_info))){
  ...
  outfile=paste(comp_info[i,1],"vs",comp_info[i,2],"DE","results.xlsx",sep="_")
  ...
  write.xlsx(volcano, file=outfile)
