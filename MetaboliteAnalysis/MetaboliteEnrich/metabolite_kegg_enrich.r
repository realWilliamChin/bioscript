...
metabolite_enrich <- function(data_table, output_prefix) {
  compound_matrix_df <- read.delim('/home/colddata/qinqiang/script/MetaboliteAnalysis/MetaboliteEnrich/Compound_kegg_matrix.txt', sep="\t", header=T, check.names = F, stringsAsFactors = F)
  ...
  kegg_rich <- enricher(
    gene = gene_select,
    TERM2GENE = compound_matrix_df[c("Ko_ID", "c_number")],
    TERM2NAME = compound_matrix_df[c("Ko_ID", "KEGG_pathway_def")],
    pvalueCutoff = 1,
    pAdjustMethod = "BH",
    qvalueCutoff = 1,
    minGSSize = 2,
  ...
  kegg_rich_result <- kegg_rich@result
  kegg_rich_result$GeneRatio <- sapply(kegg_rich_result$GeneRatio, calculate_ratio)
  kegg_rich_result$BgRatio <- sapply(kegg_rich_result$BgRatio, calculate_ratio)
  kegg_rich_result$RichFactor <- kegg_rich_result$GeneRatio / kegg_rich_result$BgRatio

  kegg_rich_result$Ko_ID <- kegg_rich_result$ID

  # 去掉 entry c_number name 列
  kegg_rich_result <- left_join(kegg_rich_result, compound_matrix_df, by = "Ko_ID")
  kegg_rich_result <- kegg_rich_result[ , !(names(kegg_rich_result) %in% c("Ko_ID", "c_number", "KEGG_pathway_def", "Compound_def"))]
  # 去重
  kegg_rich_result <- distinct(kegg_rich_result)
  kegg_rich_result <- rename(kegg_rich_result, Compound_ID = geneID)

  # 创建文件夹
  # pic_path = '/home/colddata/qinqiang/script/MetaboliteAnalysis/MetaboliteEnrich/M_entry_pictures/'

  write.xlsx(kegg_rich_result, file.path(group_output_dir, paste0(group_name, "_enrich_result.xlsx")))

  dt <- kegg_rich_result
  dt$KID <- paste(dt$ID, dt$Description, sep='_')
  ...
  if (nrow(dt) > 0) {
    p <- ggplot(dt, aes(y = factor(KID, levels = KID), x = RichFactor, size = Count, colour = pvalue)) +
      geom_point() +
      scale_size_continuous(range = c(2, 8)) +
      scale_y_discrete(limits = dt$KID) +
  ...
  dt$star <- cut(dt$pvalue, breaks = c(0, 0.001, 0.01, 0.05, Inf), 
    labels = c("***        ", "**     ", "*   ", ""))
  bar.p <- ggplot(dt, aes(x= factor(KID, levels = KID), y=RichFactor)) + 
