...
# blastp -outfmt 6 的列名(与下方 phi_blast 的 -outfmt 保持一致)
...
def phi_blast(fasta_file, output_file):
    cmd = [
        "/home/data/opt/biosoft/ncbi-blast-2.9.0+/bin/blastp",
        "-db", "/home/colddata/qinqiang/script/DenovoGenome/libs/phi/phi-base_current",
        "-query", fasta_file,
        "-out", output_file,
        "-evalue", "1e-5",
        "-num_threads", "18",
...
def add_def(blast_file, output_file):
    def_df = load_table('/home/colddata/qinqiang/script/DenovoGenome/libs/phi/phi-base_current_def.txt')
...
def main():
    args = parse_input()
    blast_file = f'{args.input}.phi.blast'
    phi_blast(args.input, blast_file)
