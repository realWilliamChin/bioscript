setwd("C:/Users/Analysis/OneDrive/Work/zhiwusuo/01_Work/2023_04_18_紫花苜蓿/Medicago_sativa-wll/Prep_files/annotation")
##### input files
...
data.kegg<-read.table("zihuamuxu-wll_KEGG_clean.txt",sep="\t",stringsAsFactors = F,header=F,quote = '')
...
BP<-read.table("zihuamuxu-wll_unigene_swiss_GO_BP_ID.txt",sep="\t",header=F,stringsAsFactors = F,quote = '')
#head(BP)
#table(head(BP[,2]))
BP.count<-data.frame(table(BP[,2]))
BP.20 <- head(BP.count[order(-BP.count[,2]),],n=20)

CC<-read.table("zihuamuxu-wll_unigene_swiss_GO_CC_ID.txt",sep="\t",header=F,stringsAsFactors = F,quote='')
CC.count<-data.frame(table(CC[,2]))
CC.20 <- head(CC.count[order(-CC.count[,2]),],n=20)
#CC.20
#?read.table
MF<-read.table("zihuamuxu-wll_unigene_swiss_GO_MF_ID.txt",sep="\t",header=F,stringsAsFactors = F,quote ="")
