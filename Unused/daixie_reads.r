setwd("/home/data/Lab_guobaolin/ZhangYixin/PLSDA_2024/20240513_petal_antho_caro/no_scale")
library(ggthemes)
library(ggplot2)
library(pheatmap)
library(reshape2)
...
if (nrow(reads_data) > 100) {
  ...
  all.heatmap<-pheatmap(reads_data,scale="row",show_rownames=F, encoding = "UTF-8")
} else {
  all.heatmap<-pheatmap(reads_data,scale="row",show_rownames=F, encoding = "UTF-8")
