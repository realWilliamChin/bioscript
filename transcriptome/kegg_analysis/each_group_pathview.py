...
def parse_input():
    ...
    p.add_argument('-f', '--files', nargs='+', required=True, 
                   help='每个样本的 reads count 文件，或者包含所有样本的 reads_matrix_filtered.txt')
...
def main():
    ...
    all_sample_file_list = args.files
    
    if len(all_sample_file_list) == 1:
        all_sample_df = pd.read_csv(all_sample_file_list[0], sep='\t')
        all_sample_list = all_sample_df.columns[1:]
        for sample in all_sample_list:
            ...
            logger.info(f'{sample} 原始行数 {source_rows_number}，过滤完 50 之后的行数 {filter_rows_number}')
            ...
            if args.kid:
                passed_path(args.kid, os.path.basename(args.kid).replace('.txt', '_passed_path.txt'))
            else:
                passed_path_df = pd.merge(result_df['KO'], path_df, on='KO', how='left')
                passed_path_df.dropna(subset=['Def']).to_csv(f'{sample}/passed_path.txt', sep='\t', index=False, header=False)
            
            result_df.drop(columns=['KO']).to_csv(f'{sample}/regulation.txt', sep='\t', index=False)
    
    elif len(all_sample_file_list) > 1:
        for sample_file in all_sample_file_list:
