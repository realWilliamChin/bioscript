...
for(i in seq_along(1:nrow(comp_info))){
  ...
  paired_flag <- (ncol(data.treat) == ncol(data.control))
  n<-NULL
  out.df<-data.frame()
  for (n in (1:nrow(reads_data))) {
    tmp.p<-t.test(x=as.numeric(data.treat[n,]),y=as.numeric(data.control[n,]),paired=paired_flag)$p.value
  ...
  total_p<-paste0(comp_info[i,1],"-vs-",comp_info[i,2],"_p")
  total_fc<-paste0(comp_info[i,1],"-vs-",comp_info[i,2],"_fc")
  ...
  out.file=paste0(comp_info[i,1],"-vs-",comp_info[i,2],".xlsx",sep="")
  ...
  write.xlsx(out.df,out.file, sheetName='Sheet1')
}
#head(total.df)
total.df<-total.df[,-1]
head(total.df)
