...
def parse_input():
    parser = argparse.ArgumentParser(description="输出文件有 reads fpkm tpm 和过滤后的 reads fpkm tpm")
...
def main():
    ...
    for sample in samples:
        ...
        df['TPM'] = df['TPM'].astype(float).round(2)
        ...
        df['FPKM'] = (df['NumReads'] * (10**9)) / (df['NumReads'].sum() * df['Length'])
        ...
        if merged_fpkm.empty:
            ...
            merged_tpm['GeneID'] = df['GeneID']
            merged_tpm[sample] = df['TPM']
        else:
            ...
            merged_tpm = pd.merge(merged_tpm, df[['GeneID', 'TPM']], how='left', on='GeneID')
            merged_tpm.rename(columns={"TPM": sample}, inplace=True)

    merged_fpkm.to_csv("fpkm_matrix.txt", sep='\t', index=False)
    merged_reads.to_csv("reads_matrix.txt", sep='\t', index=False)
    merged_tpm.to_csv("tpm_matrix.txt", sep='\t', index=False)
    ...
    merged_tpm_filter = pd.merge(merged_reads_filter[['GeneID']], merged_tpm, on='GeneID', how='left')
    ...
    merged_tpm_filter.to_csv('tpm_matrix_filtered.txt', sep='\t', index=False)
